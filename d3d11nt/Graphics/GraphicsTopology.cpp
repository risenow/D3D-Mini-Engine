...
GraphicsTopology::GraphicsTopology(GraphicsDevice& device, ShadersCollection& shadersCollection, const ShaderStrIdentifier& shaderStrIdentifier, VertexData& data, GraphicsBuffer::UsageFlags usage, bool tcEnabled, bool tbnEnabled, bool isBatch) 
    : m_TexCoordEnabled(tcEnabled), m_TBNEnabled(tbnEnabled), m_IsBatch(isBatch), 
      m_IsValid(true), m_InputLayout(device, data.GetVertexFormat(),
      shadersCollection.GetShader<GraphicsVertexShader>(shaderStrIdentifier.path, shaderStrIdentifier.variation)),
